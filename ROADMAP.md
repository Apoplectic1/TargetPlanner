...
- **Core solvers for parameter exploration** — Library `c2c1f5e` adds `Astronomy.Core.Session.SessionSolvers` with `LongestDuration` (auto-resolve) + `LongestDurationIn` (pre-resolved) + `LowestHorizon` (bisection). Foundation for downstream "what's possible tonight?" UI surfaces in TP and plan-relaxation paths in IS / ISP. `MoonClearIntersect` promoted from `private` → `internal` to let `SessionSolvers` reuse the existing moon sweep. 12 new tests, 153/153 in suite. TP UI consumers are a separate follow-up. Centered/Symmetric variants pending in the next slice.
