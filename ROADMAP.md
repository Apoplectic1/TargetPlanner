...
5. **Code-quality audit residuals** — six tiers shipped 2026-05-19 (see "Recently shipped" below for tier-by-tier commit refs). Three small unchecked items remain in [`docs/code-quality-audit.md`](docs/code-quality-audit.md): `OnGridCellClick` / `OnFilterDefaultsClick` filter-from-builtin dedup, Year/Sessions `mLastCtx` + `mLastCache` shadow fold, view-radio handler boilerplate. All low-impact; pick up opportunistically.
