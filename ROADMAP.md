...
- **Chart-package investigation — LiveCharts2 chosen** — design doc at `docs/design/chart-package-investigation.md` captures the comparison (OxyPlot / ScottPlot / LiveCharts2) against the four representative tests + the dual-target (`net481` today, `.NET 10` post-migration) requirement. LC2 wins on interaction-API maturity and no known compat regressions. Working prototype at `Prototypes/LiveCharts2Prototype/` (own git repo on `dev` branch, last commit `6613629`) demonstrates: HD overlay click → step-function bar with vertical edges, right-click restore-all, hover tooltip with proximity detection (interpolated *and* per-DataPoint snap modes with per-mode debounce), manual legend hit-test (LC2 v2.1.0-dev-365's built-in legend isn't click-aware in WinForms), legend hover tooltips, dusk/dawn `LinearGradientPaint` sections with resize-driven shader recreation, below-horizon `null` Y gaps, X-axis lock to the data range. SoC split into `CurveHitTester` / `OverlayController` / `HoverTooltipController` / `LegendClickHandler` / `LegendHitTester` — pattern playbook for the Phase 4 stateless-renderer refactor. `CurveHitTester` is library-extraction-ready (generic over T via selector lambdas, optional sentinel filter, segment-index in return) but stays in TP for now; promote to a future `Astronomy.Charts` library once a 2nd consumer materializes (XisfManager / IS / ISP / ISS) — see `~/.claude/.../memory/project_charts_library_future.md`.
