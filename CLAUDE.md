...
- **Cache worker** (`Task.Run` inside `ChartCacheStore.GetOrBuildAsync`, gated by an internal `SemaphoreSlim` cap of 4): per-target year-cache builds, per-Location `NightCache` build. Library is lock-free / managed-only since the `2249834` CS-removal — semaphore now caps actual parallel CPU work rather than a process-wide gate. **Perf budget**: post-`2249834` cache pre-population is **2–4 sec for 44 targets** on the dev machine (was ~17 min pre-CS-removal). Design future cache work against this ceiling — if total post-NINA-load time crosses ~5 sec for ~50 targets, something regressed (re-introduced lock, per-target work that should have been hoisted, etc.).
