...
3. **Hide-on-no-fit on H/D/M scrub** (the "universal" hide rule). When the active Horizon, Duration, or MoonAvoidanceProfile leaves a target with no D-hour fit, the target's curve is hidden on every chart simultaneously. Each chart hides differently; **all fit decisions live in the cache, never in the sub-charts**:
   - **Day** (single-night view, fit-tonight-only): per-target. Targets without a D-hour window tonight are excluded from `mChart.Series` and the legend entirely (not just alpha-0). The fit decision reads tonight's `NightFit` from `cache.GetFitOrNull(target, ctx.Hdm)`; per-minute altitudes come from `cache.GetDayOrNull(target, dayKey)`. H/D/M scrubs hit the coordinator → `hdmKeyChanged` → full Render after `PrepareFitsAsync`.
   - **Sky** (single-night view): per-target. Stroke alpha → 0 when no fit; restore palette alpha when fit. Same `cache.GetFitOrNull(target, ctx.Hdm)` lookup as Day. Sky's K-S magnitudes are walked inline (filter-dependent) but the fit-gate that toggles visibility is cache-driven.
   - **Year** (12-month view): per-target × per-night. `ObservablePoint.Y` → null (line break) for unfit nights; for fit nights, Y is `NightFit.Floor` read directly from the cached `TargetFitEntry`. `ChartCacheStore.ComputeNightFits` runs the `BestSession.ResolveCandidates` + `PlaceBest` + `SessionAltitude.Floor` walk under each `HdmKey`, the coordinator awaits `PrepareFitsAsync(targets, ctx.Hdm)` before dispatching Render, and the sub-chart paints synchronously.
