...
- ✅ **Library design choice deviated from the original recon**: the doc proposed *consuming NINA.Astrometry* as the math backend; we instead rolled pure-Meeus inside Astronomy.Core. Reasons that emerged after writing the doc: (a) thread-safety — NOVAS lock would have serialized scheduler workloads; (b) portability — Astronomy.Core stayed dependency-free and is consumable from any modern-.NET host without dragging NINA's WPF / Ninject / native deps (the netstandard2.0 floor that originally bridged net481 TP and net8/10 schedulers was lifted to `net10.0` in 2026-05-04 once NINA migrated upstream — see `framework_stance` memory); (c) determinism — pure managed math is straightforward to test against textbook ground truth. The "NINA adapter layer" section below is therefore obsolete — Astronomy.Core IS the analytics layer and has no NINA dependency. Plugin authors that want NINA's primitives (epoch transforms, refraction, planetary ephemerides) still call into NINA.Astrometry directly.
...
- **Portability**: every consumer (TP, XisfManager, IS desktop, ISS simulator, the planned ISP plugin hosted inside NINA) is now on .NET 10 since 2026-05-04, and Astronomy.Core targets `net10.0` to match. The original netstandard2.0 floor was load-bearing only while TP was on net481 and ISP would have been on NINA's net481 host — both dropped when NINA migrated. Astronomy.Core remains free of NINA's WPF / native deps so non-NINA consumers don't pull them transitively.
