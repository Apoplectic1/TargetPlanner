...
- **Manual smoke convention:** boot (lands on last-selected site, seed default Penns Park) → check a handful of targets → render; watch `%APPDATA%\TargetPlanner\Logs\tp.log`. Diag categories via the `TP_DIAG` env var (`Coord`, `Cache`, `Day`, `Sky`, `UI`, `Overlay`; `*` = all — Debug builds default all-on).
